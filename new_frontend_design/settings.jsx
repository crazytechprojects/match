// settings.jsx — Profile, Agent, Billing

const { useState: useStateSet } = React;

function Profile({ user, setUser }) {
  const [cur, setCur] = useStateSet('');
  const [next, setNext] = useStateSet('');
...
// ───────── Agent specs page ─────────
function AgentPage({ agent, setAgent, navigate }) {
  if (!agent) {
    return (
      <>
        <div className="page-head">
  ...
  const age = agent.dob ? Math.floor((Date.now() - new Date(agent.dob).getTime()) / (365.25 * 24 * 3600 * 1000)) : '—';
  ...
  return (
    <>
      <div className="page-head">
        ...
        <button className="btn btn-primary" onClick={() => navigate('edit-agent')}>
          Edit agent {I.arrow}
        </button>
      </div>

      <div className="section-card" style={{ display: 'flex', gap: 24, alignItems: 'center', marginBottom: 22 }}>
        <Glyph seed={agent.name} letter={agent.name[0]} size="xl" />
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 38, lineHeight: 1 }}>{agent.name}</div>
          <div style={{ color: 'var(--text-muted)', fontSize: 14, marginTop: 8 }}>
            {agent.yourGender} · age {age} · seeking {agent.theirGender?.toLowerCase()} aged {agent.ageRange[0]}–{agent.ageRange[1]}{agent.ageRange[1] === 80 ? '+' : ''}
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            <span className="pill green"><span className="dot green"></span> Active</span>
            <span className="pill">17 live conversations</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22 }}>
        <div className="section-card">
          ...
          <div className="kv" style={{ marginBottom: 18 }}>
            <div className="k">Gender</div><div className="v">{agent.yourGender}</div>
            <div className="k">Born</div><div className="v">{agent.dob || '—'}</div>
          </div>
        ...
        <div className="section-card">
          ...
          <div className="kv" style={{ marginBottom: 18 }}>
            <div className="k">Gender</div><div className="v">{agent.theirGender}</div>
            <div className="k">Age range</div><div className="v">{agent.ageRange[0]}–{agent.ageRange[1]}{agent.ageRange[1] === 80 ? '+' : ''}</div>
          </div>
          <div className="label">Looking for</div>
          <p style={{ color: 'var(--text-muted)', whiteSpace: 'pre-wrap', fontSize: 14.5, lineHeight: 1.55, margin: 0 }}>
            {agent.aboutThem || <em>None yet.</em>}
          </p>
        </div>
      </div>
    </>
...
// ───────── Billing ─────────
function Billing() {
  const [plan, setPlan] = useStateSet('plus');
...
Object.assign(window, { Profile, AgentPage, Billing });
