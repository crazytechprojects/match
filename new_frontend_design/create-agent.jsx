// create-agent.jsx — multi-step wizard

const { useState: useStateWiz } = React;

function CreateAgent({ navigate, agent, setAgent, mode = 'create' }) {
  // Steps: 0 name, 1 your gender, 2 their gender, 3 your DOB, 4 their age range, 5 about you, 6 about them, 7 review
  const [step, setStep] = useStateWiz(0);
  const [data, setData] = useStateWiz({
    name: agent?.name || '',
    yourGender: agent?.yourGender || '',
    theirGender: agent?.theirGender || '',
    dob: agent?.dob || '',
    ageRange: agent?.ageRange || [25, 35],
    aboutYou: agent?.aboutYou || '',
    aboutThem: agent?.aboutThem || '',
  });
...
Object.assign(window, { CreateAgent });
