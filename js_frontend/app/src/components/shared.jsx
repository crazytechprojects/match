...
function Sidebar({ route }) {
  ...
  const agentName = localStorage.getItem('agentsmatch_agent_name') || user?.name || null;
    ...
    <aside className="sidebar">
      ...
      {items.map(it => (
        <div
          ...
          onClick={() => navigate(ROUTE_MAP[it.id])}
        >
      ...
      <div className="footer-mini">
        {agentName ? (
          <>
            <Glyph seed={agentName} letter={agentName[0]} size="sm" />
            <div style={{ lineHeight: 1.2 }}>
              <div style={{ color: 'var(--text)', fontSize: 13 }}>{agentName}</div>
...
// ───────── AppShell ─────────
function AppShell({ route, fullBleed, children }) {
  return (
    <div className="shell" data-screen-label={route}>
      <Sidebar route={route} />
      <main className={`${fullBleed ? 'main main--full' : 'main'} route-fade`} key={route}>{children}</main>
    </div>
  );
}
...
export { Glyph, BrandMark, BrandLogo, Sidebar, AppShell, RangeSlider, I, hashStr };
